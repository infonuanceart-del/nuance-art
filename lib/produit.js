 ...
 */
import { THEME_PAR_SLUG, FORMAT_PAR_SLUG, PIECE_PAR_SLUG, CADRES } from './taxonomie.js';
...
/**
 * Cadres proposés pour l'œuvre, saisis par l'admin : nom, couleur, supplément
 * par format (`prix`, limité aux références `refs` des tailles de l'œuvre).
 * Le slug identifie le cadre dans le panier et la commande : un cadre existant
 * garde le sien quand on le renomme, un nouveau prend celui de son nom.
 * « Sans cadre » garde toujours `aucun`.
 */
function cadres(brut, refs) {
  const vus = new Set();
  const liste = [];
  for (const c of Array.isArray(brut) ? brut : []) {
    const sansCadre = c?.slug === 'aucun';
    const nom = sansCadre ? 'Sans cadre' : texte(c?.nom, 40);
    if (!nom) continue;
    let slug = 'aucun';
    if (!sansCadre) {
      const propre = /^[a-z0-9-]{1,40}$/.test(c?.slug || '') ? c.slug : slugifier(nom).slice(0, 36);
      if (!propre) continue;
      slug = propre;
      for (let n = 2; vus.has(slug) || slug === 'aucun'; n += 1) slug = `${propre}-${n}`;
    } else if (vus.has(slug)) continue;
    const hex = String(c?.hex || '').trim().toLowerCase();
    if (!sansCadre && !/^#[0-9a-f]{6}$/.test(hex)) continue;
    vus.add(slug);
    liste.push({
      slug,
      nom,
      hex: sansCadre ? 'transparent' : hex,
      supp: Math.round(nombre(c?.supp, 0, 20000)),
      prix: sansCadre ? {} : prixParFormat(c?.prix, refs),
      desc: texte(c?.desc, 80),
    });
  }
  return liste.slice(0, 12);
}

/** { "50x70": 120, … } réduit aux formats de l'œuvre ; une case vide est omise
 *  et retombe sur le calcul au périmètre (voir suppCadre). */
function prixParFormat(brut, refs) {
  const sortie = {};
  ...
  for (const ref of refs) {
    ...
    const n = Number(v);
    if (Number.isFinite(n)) sortie[ref] = Math.round(Math.min(20000, Math.max(0, n)));
  }
  return sortie;
}
...
export function normaliserProduit(entree = {}, base = null) {
  ...
  // Une fiche créée avant les cadres par œuvre n'en a pas : elle reçoit la liste par défaut.
  const choixCadres = cadres(
    entree.cadres ?? (base?.cadres?.length ? base.cadres : CADRES),
    grille.map((t) => t.ref),
  );
  if (!choixCadres.length) return { erreur: 'Proposez au moins un cadre (ou « Sans cadre »).' };
