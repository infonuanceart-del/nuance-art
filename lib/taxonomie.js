...
/** Périmètre (cm) du 50 × 70, le format auquel correspond le `supp` d'un cadre. */
const PERIMETRE_REF = 240;

/**
 * Supplément d'un cadre pour une taille donnée : une moulure se paie au mètre,
 * un grand format coûte donc plus cher à encadrer qu'un petit.
 * Le prix saisi dans l'admin pour ce format (`cadre.prix[ref]`) prime ; à
 * défaut, le `supp` du cadre est ramené au périmètre de la toile, arrondi à 10 DH.
 */
export function suppCadre(cadre, taille) {
  if (!cadre || cadre.slug === 'aucun') return 0;
  const saisi = cadre.prix?.[taille?.ref];
  if (typeof saisi === 'number' && saisi >= 0) return saisi;
  const perimetre = 2 * ((Number(taille?.l) || 0) + (Number(taille?.h) || 0));
  if (!perimetre) return cadre.supp || 0;
  return Math.round(((cadre.supp || 0) * perimetre) / PERIMETRE_REF / 10) * 10;
}

/** Cadre choisi à l'ouverture d'une fiche : sans cadre s'il est proposé, sinon le premier. */
export const cadreInitial = (liste) =>
  (liste.find((c) => c.slug === 'aucun') || liste[0])?.slug || 'aucun';
