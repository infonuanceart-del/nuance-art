...
export function etatCatalogue(produits) {
  const prix = produits.map((p) => p.prixMin || 0).filter(Boolean).sort((a, b) => a - b);
  return {
...
/**
 * Repartition du catalogue sur un champ donne — categorie, format, couleur.
 *
 * Le tableau de bord parlait uniquement de ventes : tant qu'aucune commande
 * n'est passee, il ne montrait rien. Or il y a toujours quelque chose a dire
 * sur ce qu'on vend, et c'est la premiere chose qu'on veut verifier apres
 * avoir touche au catalogue.
 */
export function repartition(produits, champ) {
  const cumul = new Map();
  for (const p of produits) {
    const cle = p[champ] || 'inconnu';
