 ...
 * @param {{sousDossier?: string}} [options]  range l'image à part (ex. les
 *        envois clients, qui ne sont pas des œuvres du catalogue)
 * @returns {Promise<{image: string, thumb: string, ratio: number,
 *          largeur: number, hauteur: number, publicId: string}>}
 */
export function envoyerImage(donnees, nom, options = {}) {
  const dossier = options.sousDossier ? `${DOSSIER}/${options.sousDossier}` : DOSSIER;
