...
import { CADRES, cadreClair, degradeCadre, suppCadre } from '@/lib/taxonomie';
 ...
 *  dans l'intitulé. `cadres` : ceux que l'admin propose pour l'œuvre ; `taille` :
 *  le format choisi, dont dépend le supplément de chaque cadre. */
export default function ChoixCadre({ valeur, onChange, taille, cadres = CADRES, titre = 'Cadre', Titre = 'h4' }) {
  const courant = cadres.find((c) => c.slug === valeur);
  const suppCourant = suppCadre(courant, taille);

  return (
    <div className="choix-cadre">
      <Titre className="lab">
        {titre} : <em>{courant?.nom}</em>
        {suppCourant > 0 && <small> (+{suppCourant} DH)</small>}
      </Titre>
      <div className="cadre-tuiles" role="radiogroup" aria-label={titre}>
        {cadres.map((c) => {
          const supp = suppCadre(c, taille);
          return (
            <button
              key={c.slug}
              type="button"
            ...
            >
              ...
              <span className="cadre-supp">{supp > 0 ? `+${supp} DH` : 'Inclus'}</span>
            </button>
          );
        })}
      </div>
